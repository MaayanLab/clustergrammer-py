...
def run_kmeans_mini_batch(df, num_samples=100, axis='row', random_state=1000):
  ...
  if type(example_label) is tuple:
    ...
    if super_string in example_label[cat_index]:
      cat_title = example_label[cat_index].split(super_string)[0]
    else:
      cat_title = 'Category'
  ...
  if found_cats:
    cat_data = generate_cat_data(cluster_data, orig_labels, num_samples)
  ...
  for i in range(num_returned_clusters):
    ...
    if found_cats:
      ...
      max_cat_name = cat_data['types'][max_cat_index]
